 ...
 */
int estim_pt_suiv_courb(Surf *s, Param_maillage *param,
			Pt* prec, P3D base[2],
  ...
  for (i = 0; i < 3; i++)
    delta[i] = h*(base[0][i] * cos(alph)
		  + base[1][i] * sin(alph));
  return deplacement_infinitesimal(s, param, prec, delta, res);
}
 ...
 * res : le résultat en sortie implicite
 */
int estim_pt_suiv_surf(Surf *s, Param_maillage* param,
 ...
 */
int deplacement_infinitesimal(Surf *s, Param_maillage* param,
			      Pt* prec, P3D delta, P3D res){
  int i;
  if (s->type == 2){
    for (i = 0; i < 3; i++)
      res[i] = prec->p[i] + delta[i];
  }
  else {
    P3D del, diff;
    int it = 0, it_max = 100;
    ...
    while (it < it_max && !term){
      if (!surf_calc_base(s->s_param, tmp.param, base, metr)){
	return 0;
      }
      det = deter(metr, 2);
	...
	return 0;
      ...
      if (!surf_setpt(s->s_param, &tmp, res)){
	return 0;
      }
 ...
 */
double iter_deplacement_infinitesimal(Surf *_s, Pt* prec,
				      P3D delta, P3D res){
  P3D base[2];
  double ps[3];
  int ind = -1;
...
double point_suivant_surface(P3D Xi1, P3D Xi2, Pt* prec,
  ...
  if (surf_newton(s->s_param, X0, fonc, fonc_p,
  ...
  if (dim == 3){
    ...
    if (!surf_setpt(s->s_param, res, tmp))
      return -20;
  }
  return theta;
}
...
int point_suivant_courbeG(Pt *Xi, double* Xi_,
  ...
  if (dim == 3){
    ...
    surf_setpt(s->s_param, res, tmp);
    if (!surf_est_dans_borne(s->s_param, res->param))
      return 0;
  }
  return 1;
}
...
int inters_double_plan_surface(Pt* Xi1, Pt* Xi2,
  ...
  for (i = 0; i < 3; i++)
    delta[i] = alpha*(base[0][i] * loc_v[0][0]
		      + base[1][i] * loc_v[1][0]);
  if (!deplacement_infinitesimal(s, param_s, Xi1, delta, X0))
    return 0;
  if (s->type == 2){
    dim = 3;
  ...
  if (dim == 2){
    res->param[0] = res->p[0];
    res->param[1] = res->p[1];
    image_cvect(s->s_param->f, res->param, res->p);
  }
  return 1;
}
 ...
 */
int calc_courbeG(CourbeG *cg, Surf *s,
  ...
  image_cvect(s->s_param->f, cg->init->param, cg->init->p);
  ...
  for (i = 1; i < nb; i++){
    ...
    param->R = h[i];
    if (i > 1){
      VECT_DEL(dir1, res[i - 2].p, res[i - 1].p, j);
    }
    else
      VECT_CPY(dir1, cg->dir_init);
    if (!surf_calc_bon_dir(s, &res[i - 1], dir1, param->base, 1)){
      PRINT_ERR_LOG(p_maill, "erreur calcul base\n");
      return i;
    }
    param->the = the[i - 1];
    if (i > 1)
      param->the *= h[i] / cg->pas;
    VECT_CPY(param->pos, res[i - 1].p);
    VECT_CPY(param->centre, res[i - 1].p);

    boo = point_suivant_courbeG(&res[i - 1], tmp, param,
				nbMax, &res[i],
...
int calc_courbePts(Surf *s, CourbePts *c, Param_maillage *param,
...
calc_maillage_init_sing(int nbptr, Pt* init,
  ...
  maillage_che(res, p_maill, s, 0, NULL, 0, ind_constr);
  return res;
}
 ...
 *   (différent de np)
 */
int ajouter_parall_diagonal(Maillage *m, Surf *s,
			    Pt* np, ListeD *lnp,
			    Param_maillage* param_s,
  ...
  if (!surf_calc_bon_dir(s, p1, dir, base, or_base)){
  ...
  if (!deplacement_infinitesimal(s, param_s, p1, del, X0)){
    free(param_new);
    return -10;
  }
  ...
  if (!deplacement_infinitesimal(s, param_s, p1,
				 init, X0)){
    free(param_new);
    return -10;
  }
...
Maillage* calc_maillage_init_sing2(int nbptr, Pt* init,
  ...
  param = create_param_new4(init, dir, _h[0], _h[1], s,
			    param_tmp1, param_tmp2,
  ...
  for (j = 0; j < 2; j++){
    ...
    point_suivant_courbeG(_pt + j, tmp_par[j],
			  ...
			  p_maill, metr, 0.0, NULL);
    if (j == 0){
      res = createMaillage(pt2, h, s);
      linit = res->lignes->l[0];
      if (ajouter_parall_diagonal(res, s, pt1, NULL,
  ...
  calc_courbe_sing(res, s, nbptr, pt2, pt1,
...
int maillage_che(Maillage *m, Param_maillage* param_s,
		 Surf *s, int bool_unique, ListeD *init,
 ...
 */
int ajoute_diag_maill(Maillage *m, Surf *s,
  ...
  while (i < nb_max && !is_term){
	...
	if (ajouter_parall_diagonal(m, s, pts[j][i], NULL, param_s,
	...
	maillage_che(m, param_s, s, 1, tmp_init, 0, ind_constr);
    ...
    maillage_che(m, param_s, s, 0, NULL, 0, ind_constr);
 ...
 * primale-duale
 */
int ajoute_courbe_dual_maillage(Maillage *m, Surf *s,
  ...
  if (c->type == 0){
    c_plan = c->c_param;
    c_surf = surf_get_courbe_from_courbeP(s->s_param, c_plan);
    ...
    param_new2 = create_param_new2(pos, NULL, pos, 0.0, 0.0, s);
  }
  ...
  while (suiv != NULL){
    ...
    if (c->type == 2)
      ind_tmp = ind_pts;
    maill_dir_tan(m->s, c, init_p, ind_tmp, pt_inter, pt_prec,
		  vect_tan);
    ...
    if (c->type == 0){
      ...
      if (!point_suivant_courbeP(s->s_param, init_p,
				 dist_courb, param_new3,
				 param_s, &res_param))
	return 0;
      courbeP_get_image(c_plan, res_param, param_tmp);
      if (!surf_setpt(s->s_param, newpt, param_tmp)){
	...
	return i + 1;
      ...
      if (!surf_calc_bon_dir(s, pt_inter, vect_tan,
			     param_new2->base, 1)){
	PRINT_ERR_LOG(param_s, "erreur calcul base\n");
	return i;
      }
	...
	break;
      ...
      param_new2->R = m->h;
      del_ind = calc_nouv_pt_courbe(s, c->c_pts->nb - ind_pts,
				    pt_suiv, pt_inter,
				    c->c_pts->pts + ind_pts,
    ...
    maillage_che(m, param_s, s, 0, suiv, 0, ind_constr);
 ...
 */
void ajoute_diag_ferme_maill(Maillage *m, Surf *s,
			     PPt *pts, int nb,
			     ListeD *init, int or, int or_vir,
			     Param_maillage* param_s,
			     int or_base, int type_maill, int ind_constr){
  ...
  int nb_aj = ajoute_diag_maill(m, s, &pts, 1, &nb, init,
				or, or_vir, -1, param_s,
				or_base, type_maill,
  ...
  if (nb_aj >= 1){
    ...
    if (!ajouter_parall_diagonal(m, s, pts[0], m->lignes->l[0],
 ...
 */
int calc_courbe_sing(Maillage *m, Surf *s,
  ...
  for (i = 1; i < nbptr; i++){
    ...
    if (i == 1 && dir_init2 != NULL){
      ...
      surf_calc_bon_dir(s, pc1, dir1, param->base, 1);
    }
     ...
     */
    if (ajouter_parall_diagonal(m, s, np_calc, NULL, p_maill,
				lsuiv, or, or_vir, -1, 1, 3, ind_constr) <= -10)
      return -10;
    if (!maillage_che(m, p_maill, s, 0, NULL, 0, ind_constr))
      return i;
    ...
    for (j = 0; j < 3; j++)
      del[j] = 2 * dir1[j] * ps / n;
    deplacement_infinitesimal(s, p_maill, pc2, del, X0);

    surf_calc_bon_dir(s, pc2, dir1, param2->base, 1);
    if (!surf_newton(s->s_param, X0, fonc, fonc_p,
 ...
 */
int calc_bezier(Surf *s, P3D pts[2], P3D der[2],
 ...
 */
int calc_cercle(Surf *s, Pt* init,
		Pt* c1, Pt* p1,
		P3D base_p1[2],
 ...
 */
int calc_diag(Maillage *m, Surf *s,
  ...
  pc1 = getvect(linit);
  surf_calc_bon_dir(s, pc1, dir, base, 1);
  ...
  if (ajouter_parall_diagonal(m, s, np_calc, NULL, p_maill,
			      ...
			      3, ind_constr) <= -10)
    return 0;
  if (!maillage_che(m, p_maill, s, 0, NULL, linit->el->ind_part, ind_constr))
    return 0;
  return 1;
}

void init_inter_droite_sphere(Surf *s, Param_maillage *param,
			      P3D init, P3D p1,
...
void deplacer_courbe(Surf *s, Pt *pts, int nbpts, Pt *p,
...
void ajout_courbe_dual(Maillage *m, Surf *s, PPt *pts, int nbpts,
  ...
  ajoute_diag_maill(m, s, &pts, 1, &nbpts, init, 2,
 ...
 */
void jonction_dual_pts(Maillage *m, Surf *s, int nb_dual,
  ...
  if (param == NULL)
    return;
  maillage_set_sing(m, init);
  ajoute_diag_maill(m, s, npts, nb_dual, nbpts, init, 0,
		    or_vir, or_voistopt, param_s,
		    or_base, 3, callback_sing, param, fermer_sing, ind_constr);
}
 ...
 */
int calc_nouv_pt_courbe(Surf *s, int nb, Pt *p1, Pt* p_prec,
  ...
  if(!nouv_pt_courbe(s, p1->p, pos, del, pas_centre,
		     param_new, res, param_m))
    return -10;
  return j - 1;
}

int nouv_pt_courbe(Surf *s, double *centre, Pt *pos,
...
void maill_dir_tan(Surf *s, Courbe *c, double param, int ind,
  ...
  if (c->type == 0){
    ...
    image_cvect_p(s->s_param->f, par, tmp2);
...
void maill_dir_tan_init(Surf *s, Courbe *c, double *dir_tan){
