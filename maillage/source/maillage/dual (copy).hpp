...
void constr_dual(Maillage *m, Param_maillage *param){
  ...
  /*  if (m->lignes->nb_sing[0] > 0)
      ...
      init = get_element_gtab(m->lignes->mailles[0], 1);
  ...
  if(init->saut != NULL){
    ...
    /* TODO : prendre en compte le cas où le voisin est une 
     *        singularité ??????? */
  }
  while (elbord != NULL){
    PRINT_WAR_LOG(param, "changement el liste\n");
    pts[0] = elbord->el;
    el_prim = pts[0]->el->el;
	...
	if (vois != NULL){
	  ...
	  if (vois->vois != NULL){
	    ...
	    ind_vois2 = ind_vois;
	    if(ind_vois[0] >= 0){
	      vois_tmp = liste_getVois(vois, ind_vois[0]);
	      if(vois_tmp != NULL){
		del_sing = get_saut_sing(el_prim, vois_tmp);
		if((vois_tmp->el->dir_sing+2+del_sing)%4 
		   != (or + 3) % 4){
		  ind_vois[1] = ind_vois[0];
		  ind_vois[0] = -1;
		} else ind_vois2++;
	      }
	    }
	    ind_vois2[0] = el_prim->el->pos_sing - 1;
	    if (ind_vois2[0] < 0 && vois->vois->est_ferme)
	      ind_vois2[0] = vois->vois->nb_vois - 1;
	    if(ind_vois2[0]>=0){
	      vois_tmp = liste_getVois(vois, ind_vois2[0]);
	      if(vois_tmp != NULL){
		del_sing = get_saut_sing(el_prim, vois_tmp);
		if(ind_vois2 == ind_vois && ind_vois[1] == -1){
		  if((vois_tmp->el->dir_sing+2+del_sing) 
		     != (or + 3) % 4){
		    ind_vois[1] = ind_vois[0];
		    ind_vois[0] = -1;
		  } 
		...
		if (ind_quadr_tmp == NULL){
		  ...
		  pts[1] =
		    ajouter_listeD(m->pts_dual,
				   pts[0], vois2, NULL, ind_aj,
				   ind_aj_new,
				   vois2->vois != NULL, 0, 
				   vois2->el->primal_pos_sing);
		  if(vois2->vois != NULL)
...
int dual_autour_sing(PListeD *pts, GTab *gtab_bool, 
		     GTab *quadr, int dir_suiv, int *tot_pts, int *bool_aj_sing){
  int i;
  ListeD *primal;
  ListeD *sing = NULL;
  for(i = 0;i<3;i++){
  ...
  if(sing->vois->est_ferme){
    ...
    while(tmp != NULL && tmp != pts[0]){
      tmp = liste_getVois(tmp, dir_suiv);
      if(tmp == NULL || tmp->el->pos_sing == -1)
	break;
      pts[nb_tot] = tmp;
      nb_tot++;
...
Saut_vois* dual_copier_saut(Saut_vois *s){
  if (s == NULL)
    return NULL;
  return create_saut_vois(s->dir1, s->delta);
}
